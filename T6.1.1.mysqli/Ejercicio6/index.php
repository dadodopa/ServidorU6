...
class ConexionBD
{
    ...
    public function addTabla($nombreTabla){
        $conexionEstablecida = $this->abrir();
        $sentecia = "CREATE TABLE  IF NOT EXISTS $nombreTabla 
                (id INT PRIMARY KEY AUTO_INCREMENT,
                nombre VARCHAR(50))";
        if ($conexionEstablecida->query($sentecia)) {
            echo "Se ha creado la tabla con el nombre: $nombreTabla";
        } else{
            echo "Error al añadir la columna: ". $conexionEstablecida->error;
        }
    }
...
$pruebaConexion->addTabla("nuevatabla");